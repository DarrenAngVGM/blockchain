"""Changes:
Added a fourth user: D!
Made the public key related to private key: number / sum of digits. Bad hash is the same.
For proof of work, simulate how a miner might guess a SHA256 puzzle; just guess until it's right.
Moved generate_keys() into the Ledger class too.

Most importantly, Ledger.transaction() now demands proof of work. That is, it needs the correct nonce delivered.

At this stage, assume that there is user consent on both sides (authenticate()) and user-initiated payments ((
user_initiated_payment()). Each transaction is treated as one block. For a transaction to succeed, there must be valid
proof of work."""
...
def main():
    ...
    df = pd.read_csv("06-ledger_genuine.csv", index_col=0)
    ledger = Ledger(df)
...
def proof_of_work(newest_hash, nonce):
    """A very simple proof of work: nonce to be solved for us the exponent/power which makes the result round off to 2.
     Definitely has more than one solution, definitely math-able. But I'm going to guess anyway."""
    mine_attempt = newest_hash ** nonce
    if round(mine_attempt, 0) == 2:
        return True
    else:
        return False
...
class Ledger:
    ...
    def generate_blockchain(self):
        ...
        blockchain = [1.2]  # Beyond the first row (the "genesis block") start hashing. Hash for genesis block is 1.2.
